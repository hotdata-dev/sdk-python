...
# **list_query_runs**
> ListQueryRunsResponse list_query_runs(limit=limit, cursor=cursor, status=status)
...
with hotdata.ApiClient(configuration) as api_client:
    ...
    try:
        # List query runs
        api_response = api_instance.list_query_runs(limit=limit, cursor=cursor, status=status)
